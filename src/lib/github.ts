...
function prBody(analysis: AnalysisResult): string {
  ...
  return [
    ...
    '',
    `### What this PR adds`,
    `- \`${FIX_FILENAME.faq}\` — answer-ready FAQ block.`,
    `- \`${FIX_FILENAME.comparison_page}\` — Markdown comparison page.`,
    `- \`${FIX_FILENAME.schema}\` — JSON-LD for a \`<script type="application/ld+json">\` block.`,
    '',
    `### Review checklist`,
    `- [ ] Replace bracketed placeholders (e.g. \`[X%]\`) with real numbers.`,
    `- [ ] Verify factual claims in the FAQ.`,
    `- [ ] Check competitor claims for fairness.`,
    `- [ ] Decide where the comparison page should live in your routing.`,
    `- [ ] Wire the JSON-LD into the relevant page \`<head>\`.`,
    '',
